...
The launcher and direct wrapper execution need PowerShell, `Az.Accounts`, `Az.Resources`, and access to Azure; neither requires Azure CLI or a separately maintained ARM JSON template. Direct wrapper execution also needs the standalone Bicep executable available to Az PowerShell. `Invoke-SkillableDay2.ps1` authenticates using Az PowerShell and verifies that the active subscription matches `SubscriptionId` before downloading the repository. The launcher also requires access to GitHub's archive endpoint.

Before downloading the repository, the launcher checks for `bicep` on PATH and prints its version if available, without downloading another copy. If the runtime does not provide it, the launcher bootstraps the latest official standalone Bicep CLI from the `Azure/bicep` GitHub releases (`bicep-win-x64.exe` for Windows x64 or `bicep-linux-x64` for Linux x64). This requires access to GitHub release downloads and their redirected asset hosts. The binary is installed under the launcher's temporary working directory, marked executable on Linux, and added to PATH for the current process only. The launcher verifies `bicep --version` before proceeding and stops with a clear error if it cannot execute successfully. No administrator rights, package managers, machine-level PATH changes, Azure CLI installation, or customer-file changes are required. The original process PATH is restored and the temporary binary is removed with the working directory in the `finally` block.
